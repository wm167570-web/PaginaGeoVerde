...
export default function Home() {
  ...
  return (
    <main className="relative font-sans antialiased bg-white overflow-x-hidden">
      ...
      <section id="resources" className="py-24 bg-brand-forest text-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-20 items-center">
            <div>
              <h2 className="text-[61px] font-bold uppercase tracking-[0.3em] opacity-60 mb-6">Biblioteca GeoVerde</h2>
              ...
              <div className="flex flex-col gap-6">
                {content.resources.map((resource:any, i:number) => (
                    ...
                    onClick={(e) => { 
                      ...
                      if (resource.id === 'calculator') {
                        e.preventDefault();
                        setIsCalculatorOpen(true);
                      } else if (resource.id === 'latest_video' || (i === 1 && !resource.id)) { // Fallback if no ID is present
                        fetchLatestVideo(e, resource.link);
                      }
                    }}
                    target="_blank"
                    rel="noopener noreferrer"
                    whileHover={{ x: 20 }}
                    className={`group flex items-center justify-between p-6 bg-white/10 backdrop-blur-md rounded-2xl border ${selectedResource === i ? 'border-brand-primary ring-2 ring-brand-primary' : 'border-white/20'} cursor-pointer ${loading && resource.id === 'latest_video' ? 'opacity-50 cursor-wait' : ''}`}
                  >
                    <div className="flex items-center gap-6">
                       <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center">
                          {loading && resource.id === 'latest_video' ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-white"></div> : resource.id === 'calculator' ? <Calculator className="w-6 h-6" /> : <Download className="w-6 h-6" />}
                       </div>
                       <div>
                         <span className="text-[10px] uppercase font-bold tracking-widest opacity-60 mb-1 block">{resource.type}</span>
                         <span className="text-lg font-bold group-hover:text-brand-primary transition-colors">{loading && resource.id === 'latest_video' ? 'Buscando...' : resource.title}</span>
                       </div>
                    </div>
      ...
      <Footer />
      <BackToTop />
      <CarbonCalculator isOpen={isCalculatorOpen} onClose={() => setIsCalculatorOpen(false)} />
    </main>
  );
}
